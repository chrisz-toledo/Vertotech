...
const EmployeeForm: React.FC<Omit<EmployeeFormProps, 'onDeleteRequest'>> = ({ onSave, editingEmployee, onCancel, jobRoles, onAddJobRole }) => {
  ...
  const steps = [
      ...
      { number: 2, label: 'Detalles y Pago' },
      { number: 3, label: 'Calificaciones' },
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        {/* Progress Bar */}
        <div className="flex items-center justify-center p-2 rounded-lg bg-gray-100/80 dark:bg-gray-900/80 border dark:border-gray-700">
            {steps.map((step, index) => (
                <React.Fragment key={step.number}>
                    <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${currentStep >= step.number ? 'bg-blue-600 text-white' : 'bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-200'}`}>
                            {step.number}
                        </div>
                        <span className={`font-semibold text-sm ${currentStep >= step.number ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500'}`}>{step.label}</span>
                    </div>
                    {index < steps.length - 1 && <div className="flex-1 h-1 bg-gray-300 dark:bg-gray-600 mx-4 rounded-full"></div>}
                </React.Fragment>
            ))}
        </div>
        ...
        {currentStep === 1 && (
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pb-6 border-b dark:border-gray-700">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">{t('jobStatus')}</label>
                        <div className="flex items-center gap-4">
                            <button type="button" onClick={() => setFormData(p => ({...p, isActive: true}))} className={`flex-1 px-4 py-3 font-semibold rounded-lg border-2 transition-all ${formData.isActive ? 'bg-emerald-600 text-white border-emerald-600' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:border-emerald-500'}`}>{t('active')}</button>
                            <button type="button" onClick={() => setFormData(p => ({...p, isActive: false}))} className={`flex-1 px-4 py-3 font-semibold rounded-lg border-2 transition-all ${!formData.isActive ? 'bg-gray-500 text-white border-gray-500' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 border-gray-300 dark:border-gray-600 hover:border-gray-500'}`}>{t('inactive')}</button>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                    ...
                    <div>
                        ...
                        <select name="role" value={formData.role} onChange={handleChange} className="w-full p-3 bg-white dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded-lg">
                            <option value="Labor">Labor</option> <option value="Foreman">Foreman</option> <option value="Project Manager">Project Manager</option> <option value="Owner">Owner</option>
                        </select>
                    </div>
        ...
        {currentStep === 3 && (
            <div className="space-y-4">
                <h3 className="text-lg font-semibold dark:text-gray-200">{t('detailedRating')}</h3>
                <div className="space-y-4">
                    ...
                    <div className="space-y-4 p-4 bg-gray-50 dark:bg-gray-700/50 border dark:border-gray-600 rounded-lg">
                        <InputField id="specialtyDescription" name="specialtyDescription" label={t('specialtyArea')} value={formData.specialtyDescription} onChange={handleChange} placeholder={t('specialtyPlaceholder')} icon={<StarIcon className="w-5 h-5" />} optional />
                        <RatingInput label={t('rateSpecialty')} value={formData.rating.specialty} onRate={(v) => setFormData(p => ({...p, rating: {...p.rating, specialty: v}}))} />
                    </div>
                </div>
            </div>
        )}
             ...
             <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-48 overflow-y-auto pr-2">
                    {formData.documents?.map(doc => (
                        <div key={doc.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border dark:border-gray-600">
                            <div className="flex items-center gap-3"><FileIcon className="w-5 h-5 text-gray-500 flex-shrink-0" /><div className="truncate"><span className="text-sm font-medium text-gray-700 dark:text-gray-200 truncate">{doc.name}</span><p className="text-xs text-gray-500 dark:text-gray-400">{doc.type}{doc.expirationDate ? ` - Expires: ${doc.expirationDate}`: ''}</p></div></div>
                ...
                <div className="p-3 bg-gray-50 dark:bg-gray-700/50 border dark:border-gray-600 rounded-lg space-y-3">
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="text-sm font-medium dark:text-gray-300">{t('docType')}</label><select value={newDocDetails.type} onChange={e => setNewDocDetails(p => ({...p, type: e.target.value as any}))} className="w-full p-2 border dark:border-gray-600 rounded mt-1 bg-white dark:bg-gray-700 dark:text-white"><option value="certification">{t('certification')}</option><option value="license">{t('license')}</option><option value="insurance">{t('insurance')}</option><option value="w9">{t('w9')}</option><option value="other">{t('other')}</option></select></div>
                        <div><label className="text-sm font-medium dark:text-gray-300">{t('expirationOptional')}</label><input type="date" value={newDocDetails.expirationDate || ''} onChange={e => setNewDocDetails(p => ({...p, expirationDate: e.target.value}))} className="w-full p-2 border dark:border-gray-600 rounded mt-1 bg-white dark:bg-gray-700 dark:text-white" /></div>
                    </div>
                    <button type="button" onClick={() => docFileInputRef.current?.click()} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 hover:border-blue-500"><PaperclipIcon className="w-5 h-5" /><span>{t('attachDocument')}</span></button>
                </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t dark:border-gray-700">
            <div className="flex items-center gap-4">
                {currentStep > 1 && <button type="button" onClick={() => setCurrentStep(s => s - 1)} className="glass-button-secondary">Anterior</button>}
                {currentStep < steps.length && <button type="button" onClick={() => setCurrentStep(s => s + 1)} className="glass-button">Siguiente</button>}
                {currentStep === steps.length && <button type="submit" disabled={!isFormValid || isAddingNewRole} className="glass-button">{editingEmployee ? t('saveChanges_btn') : t('addEmployee_btn')}</button>}
