...
import type { Estimate, Client, FinancialSettings } from '../../types';
...
export const EstimateModal: React.FC<EstimateModalProps> = ({ isOpen, onClose, editingEstimate, onSave, clients, financialSettings }) => {
    const { t } = useTranslation();
    
    if (!isOpen) return null;

    const handleSave = (data: Omit<Estimate, 'id' | 'estimateNumber' | 'createdAt' | 'deletedAt'> | Estimate, id?: string) => {
        onSave(data, id);
        onClose();
    };
            ...
            >
                <header className="flex items-center justify-between p-5 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                    <div className="flex items-center gap-3">
                        <CalculatorIcon className="w-6 h-6 text-gray-600 dark:text-gray-300"/>
                        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                            {editingEstimate ? 'Edit Estimate' : t('addEstimate')}
                        </h2>
                    </div>
                ...
                <main className="overflow-y-auto bg-white dark:bg-gray-800/50">
                    <div className="p-6 sm:p-8">
                        <EstimateForm
                            ...
                            clients={clients}
                            defaultTaxRate={financialSettings.defaultTaxRate}
                        />
                    </div>
                </main>
            </div>
