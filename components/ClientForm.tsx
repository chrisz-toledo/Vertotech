    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t dark:border-gray-700">
        <div className="flex gap-4">
          <button type="submit" name="save" disabled={!isFormValid} className="px-8 py-3 font-semibold text-white bg-primary rounded-lg disabled:bg-gray-400">{editingClient ? t('saveChanges_btn') : t('addClient')}</button>
        ...
        {editingClient && onDeleteRequest && (<button type="button" onClick={handleDelete} className="flex items-center gap-2 px-6 py-3 font-semibold text-rose-600 bg-rose-100 dark:bg-rose-900/50 dark:text-rose-300 rounded-lg"><TrashIcon className="w-5 h-5" />{t('moveToTrash')}</button>)}
      </div>
    </form>
  );
};
