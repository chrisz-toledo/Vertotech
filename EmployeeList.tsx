...
import { BulkActionBar } from './components/new/BulkActionBar';
    ...
    <>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-2">
            {/* FIX: Use glass-button-secondary for consistent styling */}
            <button onClick={() => openFilterPanel('employee')} className="glass-button-secondary">
                <FilterIcon className="w-5 h-5" />
                <span className="hidden sm:inline">{t('filters')}</span>
            </button>
            <ViewSwitcher viewMode={viewMode} setViewMode={(mode) => setViewMode('employee', mode)} />
            {/* FIX: Use glass-button for consistent styling */}
            <button onClick={() => openPeekPanel('employee')} className="glass-button">
